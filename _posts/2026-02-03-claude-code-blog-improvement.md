...
요즘 코딩할 때 AI 없이 하면 맨손으로 못질하는 느낌이다. 이번에 블로그를 좀 손보려고 했는데, Anthropic의 [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview)를 써봤다. 터미널에서 바로 돌아가는 CLI 에이전트인데, 결론부터 말하면 꽤 괜찮았다.
...
그래서 `CLAUDE.md` 파일을 만들었다. 이 파일은 Claude Code가 세션 시작 시 자동으로 읽는 프로젝트 컨텍스트 문서다. 여기에 빌드 명령어, 아키텍처, 주요 설정 등을 적어두면 새 세션에서도 "아 이건 Jekyll 4.4.1 블로그고, Hamilton 테마 쓰고, 다크모드는 이렇게 동작하고..." 하고 파악한 상태에서 시작한다.
...
터미널에서 자연어로 "블로그 뭐 개선하면 좋을까?"하고 물어보면 분석하고 계획 세우고 구현까지 해주는 게 확실히 신기하긴 하다. 특히 레포 전체를 컨텍스트로 잡고 작업하는 점이 단순 코드 생성 도구와의 차이점이다.

...
처음에 `/init` 명령어를 통해 `CLAUDE.md` 파일을 만들었다. 코드베이스를 알아서 분석한 뒤 적절한 내용을 채워 넣어 준다. 이 파일은 Claude Code가 세션 시작 시 자동으로 읽는 프로젝트 컨텍스트 문서다. 여기에 빌드 명령어, 아키텍처, 주요 설정 등이 작성되면 새 세션에서도 "아 이건 Jekyll 블로그고, 무슨 테마 쓰고, 다크모드는 이렇게 동작하고..." 등등을 파악한 상태에서 시작한다.
